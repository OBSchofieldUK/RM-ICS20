...
class droneCore():
    def __init__(self):
        ...
        self.homePub = rospy.Publisher(
            homePubTopic, mavros_msgs.msg.HomePosition, queue_size=1)
        self.messageHandlerPub = rospy.Publisher(
            '/onboard/messageEnable', Bool, queue_size=1)
        ...
        rospy.Subscriber(mavros.get_topic('global_position',
                                          'global'), NavSatFix, self._cb_SatFix)
    ...
    def _cb_onKeypress(self, msg):
        ...
        if keypress == 't':
            ...
            if self.sysState == 'takeoff':
                self.droneLoiter()
        if keypress == 'r':
            if not(self.uavState.mode == 'OFFBOARD'):
                rospy.logwarn(
                    "autopilot enable attempt but offboard not running.")
            else:
                self.setState('apAlign')
        if keypress == 'l':
